...
def window_partition(x, window_size: int): # 把图片拆成窗口
    """
    Args:
        x: (B, H, W, C)
...
def window_reverse(windows, window_size: int, H: int, W: int): # 从窗口恢复图片
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
...
class WindowAttention(nn.Module): # 实现窗口内注意力
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        # 定义一个可学习的位置偏置表，大小为[(2w-1)(2h-1), heads]，之后会对一个窗口内的patch embedding根据某规则运算得到一个位置相关的索引值，在这个表中查找对应的偏置值
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # get pair-wise relative position index for each token inside the window
        # 位置编码，通过如下运算得到每个位置的相对位置索引值，定为一个固定的参数
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)  N=Wh*Ww
...
class SwinTransformerBlock(nn.Module): # 实现W-Att或SW-Att
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size > 0: # 在shift window时需要定义mask，具体？
    ...
    def forward(self, x):
        ...
        # cyclic shift
        if self.shift_size > 0: # 将图片向左上方滚动，超出的部分再补回到右下角，相当于移动窗口
...
class PatchMerging(nn.Module): # 降低分辨率，增加通道维度
...
class BasicLayer(nn.Module): # swin基础层，每层由n个W-Att模块和n个SW-Att模块交替构成，再接一个patch merging层
...
class SwinTransformer(nn.Module): # 整体模型
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, weight_init='', **kwargs): # 每层(S)W-Att总数分别为2、2、6、2；随着层数增加，特征维度加大，num_heads也在倍增
        super().__init__()
