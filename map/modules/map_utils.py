...
def set_metrics(pl_module): # 给pl_module设置一些计算指标函数
...
def set_schedule(pl_module):
    ...
    gaussian_names = ['gau_mu_encoder', 'gau_var_encoder', 'gau_nm1', 'gau_nm2', 'gau_encoder']
    head_names = ["vqa_classifier", "nlvr2_classifier", "mlm_score", "itm_score", "snli_classifier", "tdiuc_classifier"]
    cross_modal_names = ['cross_modal']
    # head_names = head_names + gaussian_names
    # cross_modal_names = cross_modal_names + gaussian_names
    lr_mult_head = pl_module.hparams.config["lr_mult_head"]
    lr_mult_cross_modal = pl_module.hparams.config["lr_mult_cross_modal"]
    lr_mult_gaussian = pl_module.hparams.config["lr_mult_gaussian"]
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                ...
                and not any(gt in n for gt in gaussian_names)
            ],
            "weight_decay": wd,
            "lr": lr,
        },
        ...
        },
        {
            "params": [
                ...
                and not any(bb in n for bb in head_names)
                and not any(ht in n for ht in cross_modal_names)
                and any(gt in n for gt in gaussian_names)
            ],
            "weight_decay": wd,
            "lr": lr * lr_mult_gaussian,
        },
        {
            "params": [
                p
